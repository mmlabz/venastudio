...
class _OrdersPageState extends ConsumerState<OrdersPage>
          ...
          children: [
            ...
            if (isEmployee) ...[
              ...
              _orderInfoRow(
                ...
                label: 'Services',
                value: _serviceSummaryText(order),
              ),
              const SizedBox(height: 7),
              _orderInfoRow(
                icon: Icons.timer_outlined,
                label: 'Time Taken',
                value: _durationText(order),
                highlight: true,
              ),
            ],
            const Spacer(),
  ...
  String _serviceSummaryText(Map<String, dynamic> order) {
    final rawItems = order['orderItems'];
    final items = rawItems is List ? rawItems : const [];

    if (items.isEmpty) return '0 services';
    ...
    for (final raw in items) {
      ...
      final qty = int.tryParse('${raw['items'] ?? raw['quantity'] ?? 1}') ?? 1;
      quantity += qty < 1 ? 1 : qty;
      final name = '${raw['name'] ?? ''}'.trim();
      if (name.isNotEmpty && names.length < 2) names.add(name);
    }

    final count = quantity == 0 ? items.length : quantity;
    if (names.isEmpty) return '$count service${count == 1 ? '' : 's'}';

    final suffix = count > names.length ? ' +${count - names.length} more' : '';
    return '${names.join(', ')}$suffix';
  }

  String _durationText(Map<String, dynamic> order) {
    final apiDuration = '${order['duration'] ?? ''}'.trim();
    if (apiDuration.isNotEmpty) return apiDuration;

    final start = _parseOrderDate(order['startTime'] ?? order['start_time']);
    final end = _parseOrderDate(order['endTime'] ?? order['end_time']);

    if (start != null && end != null && !end.isBefore(start)) {
      final minutes = end.difference(start).inMinutes;
      if (minutes > 0) return _formatMinutes(minutes);
    }
    ...
    for (final raw in items) {
      if (raw is! Map) continue;
      estimatedMinutes += int.tryParse('${raw['time'] ?? 0}') ?? 0;
    }

    if (estimatedMinutes > 0) return '${_formatMinutes(estimatedMinutes)} est.';
    return 'Not recorded';
  }
